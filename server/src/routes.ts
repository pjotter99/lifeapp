...
export function registerRoutes(app: FastifyInstance): void {
  ...
  // Kategorie-Ansicht (Auswertung): Ausgaben eines Monats nach Ober- und
  // Unterkategorie, inkl. Einzelbuchungen — komplette Hierarchie in einem
  // Aufruf, damit das Aufklappen im Frontend ohne Nachladen funktioniert.
  // Transfers und Einnahmen sind ausgeschlossen (nur amount_cents < 0).
  app.get<{ Querystring: { month?: string } }>('/api/summary/categories', async (request, reply) => {
    const month = request.query.month ?? today().slice(0, 7);
    if (!/^\d{4}-\d{2}$/.test(month)) {
      return reply.code(400).send({ error: 'month muss YYYY-MM sein.' });
    }
    ...
    const topCategories = db
      .prepare<
        [string, string],
        { top_id: number; top_name: string; amount_cents: number }
      >(
    ...
    const categories = topCategories.map((top) => {
      const subcategories = db
        .prepare<
          [number, string, string],
          { id: number; name: string; amount_cents: number }
        >(
        ...
        .all(top.top_id, monthStart, nextMonthStart);

      const subcategoriesWithTransactions = subcategories.map((sub) => {
        const transactions = db
          .prepare<
            [number, string, string],
            { id: number; date: string; amount_cents: number; payee: string | null; note: string | null }
          >(
          ...
          .all(sub.id, monthStart, nextMonthStart);
    ...
    return { month, total_cents: totalCents, categories };
  });
